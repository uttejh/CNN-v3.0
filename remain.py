...
weights_FC = p.initWeights(640,numOfHiddenNeurons1) # FC.shape[0]=640

# Initialising weights of Hidden Layer
weights_HL1 = p.initWeights(numOfHiddenNeurons1, numOfHiddenNeurons2)

weights_HL2 = p.initWeights(numOfHiddenNeurons1, numOfOutputNeurons)
...
for iterat_epoch in range(epochs):
	...
	for iterat_image in range(numofInputImages):
		...
		# dw - Change in weight
		dweight_output = numpy.outer(sigmoid_hidden_values2, d_output)

		# + because -*- = +
		weights_HL2 = weights_HL2 + alpha*(dweight_output)

		bhl2 += alpha*numpy.sum(d_output) 

		# --------------------- Hidden Layer 1 <-- Hidden Layer 2 -----------------------------------

		slope_hidden_layer2 = derivative(sigmoid_hidden_values2)

		error_hidden_layer2 = numpy.dot(weights_HL2, d_output)

		# err_hl.append(numpy.sum(0.5*error_hidden_layer2**2))

		d_hidden_layer2 = error_hidden_layer2*slope_hidden_layer2

		dweight_hidden2 = numpy.outer(sigmoid_hidden_values1, d_hidden_layer2)

		weights_HL1 = weights_HL1 + alpha*(dweight_hidden2)

		bhl1 += alpha*numpy.sum(d_hidden_layer2)


		# -------------------------------- FC <-- Hidden Layer -----------------------------------

		slope_hidden_layer = derivative(sigmoid_hidden_values1)

		error_hidden_layer = numpy.dot(weights_HL1, d_hidden_layer2)
...
file_HL_to_output1 = open("./weights/HL_to_output1.txt", "w")
pickle.dump(weights_HL1, file_HL_to_output1)
file_HL_to_output1.close()

file_HL_to_output2 = open("./weights/HL_to_output2.txt", "w")
pickle.dump(weights_HL2, file_HL_to_output2)
file_HL_to_output2.close()
...
file_bhl1 = open("./weights/bhl1.txt", "w")
pickle.dump(bhl1, file_bhl1)
file_bhl1.close()

file_bhl2 = open("./weights/bhl2.txt", "w")
pickle.dump(bhl2, file_bhl2)
file_bhl2.close()
